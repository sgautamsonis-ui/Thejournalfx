...
import { Building2, Brain, Check, ChevronRight, Landmark, Layers3, Palette, Pencil, Plus, Save, SlidersHorizontal, Target, Trash2, UserRound, X } from "lucide-react";
...
const EMPTY_ACCOUNT = { name: "", broker: "", account_type: "Live", balance: 10000, currency: "USD", dailyLimit: "", weeklyLimit: "", propFirmType: "", maxDrawdown: "", profitTarget: "" };
...
export default function Settings() {
  ...
  const saveLimits = async (id, source) => {
    ...
    if (source.propFirmType) clean.propFirmType = source.propFirmType;
  ...
  const addAccount = async () => {
    ...
    try {
      const { dailyLimit, weeklyLimit, propFirmType, maxDrawdown, profitTarget, ...payload } = newAccount;
      const created = await accountsApi.create({ ...payload, balance: Number(payload.balance) || 0 });
      await saveLimits(created.id, { dailyLimit, weeklyLimit, propFirmType, maxDrawdown, profitTarget });
  ...
  const startEdit = account => {
    ...
    setEditing({ ...account, dailyLimit: limits.daily ?? "", weeklyLimit: limits.weekly ?? "", propFirmType: limits.propFirmType ?? "", maxDrawdown: limits.maxDrawdown ?? "", profitTarget: limits.profitTarget ?? "" });
  };
  const updateAccount = async () => {
    if (!editing?.name?.trim()) return toast.error("Account name is required");
    try {
      const { id, dailyLimit, weeklyLimit, propFirmType, maxDrawdown, profitTarget, ...payload } = editing;
      const updated = await accountsApi.update(id, { ...payload, balance: Number(payload.balance) || 0 });
      await saveLimits(id, { dailyLimit, weeklyLimit, propFirmType, maxDrawdown, profitTarget });
