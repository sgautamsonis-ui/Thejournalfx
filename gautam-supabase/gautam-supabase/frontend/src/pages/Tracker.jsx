...
function DrillDialog({ drill, onClose, navigate }) { return <Dialog open={!!drill} onOpenChange={(open) => !open && onClose()}><DialogContent className="max-w-4xl bg-white"><DialogHeader><DialogTitle>{drill?.title}</DialogTitle></DialogHeader><div className="flex justify-between items-center text-xs text-[#6D6D82]"><span>{drill?.trades?.length || 0} matching closed trades</span><button onClick={() => { onClose(); navigate("/trades"); }} className="font-semibold text-[#7C3AED] hover:underline">Open Trade View</button></div><div className="max-h-[58vh] overflow-auto rounded-xl border border-[#E8E8F1]"><table className="w-full min-w-[650px] text-sm"><thead className="sticky top-0 bg-[#FAFAFD] text-left text-[#6D6D82]"><tr><th className="p-3 font-medium">Date</th><th className="p-3 font-medium">Symbol</th><th className="p-3 font-medium">Direction</th><th className="p-3 font-medium">Strategy</th><th className="p-3 font-medium">R</th><th className="p-3 font-medium text-right">P&L</th></tr></thead><tbody>{(drill?.trades || []).map((trade) => <tr key={trade.id} className="border-t border-[#F0F0F5] hover:bg-[#FBF8FF]"><td className="p-3 tjfx-mono text-xs">{trade.date || "—"}<br/><span className="text-[#8B8B9A]">{trade.entry_time || ""}</span></td><td className="p-3 font-bold">{trade.symbol || "—"}</td><td className="p-3 capitalize">{trade.direction || "—"}</td><td className="p-3 max-w-[180px] truncate">{trade.strategy || "—"}</td><td className="p-3 tjfx-mono">{trade.r_multiple !== null && trade.r_multiple !== undefined ? `${safeNumber(trade.r_multiple).toFixed(2)}R` : "—"}</td><td className={`p-3 text-right tjfx-mono font-bold ${tone(safeNumber(trade.net_pnl))}`}>{money(safeNumber(trade.net_pnl))}</td></tr>)}</tbody></table></div></DialogContent></Dialog>; }
